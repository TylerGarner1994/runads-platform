...
/**
 * Call Claude with skill-enriched system prompt, falling back to base prompt on failure.
 * Use this for any call that injects skill context — ensures generation never fails
 * just because the enriched prompt is too large or triggers an API error.
 *
 * @param {Object} options - Same as callClaude, plus:
 * @param {string|null} options.baseSystemPrompt - Fallback system prompt without skill context
 */
export async function callClaudeWithFallback({
  ...
  try {
    return await callClaude({ systemPrompt, ...rest });
  } catch (err) {
    // If we have a fallback and the full prompt failed, retry without skill context
    if (baseSystemPrompt !== null && systemPrompt !== baseSystemPrompt) {
      console.warn(`Claude call failed with skill context, retrying without. Error: ${err.message}`);
      return await callClaude({ systemPrompt: baseSystemPrompt, ...rest });
    }
    throw err;
  }
}
