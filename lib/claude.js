...
// Unified wrapper for all Claude API calls with retry logic and timeout
...
/**
 * Call Claude API with retry logic, timeout, and consistent error handling.
 *
 ...
 * @param {number} [options.timeoutMs] - AbortController timeout in ms (default: 90000)
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        lastError = new Error(`Claude API timeout after ${timeoutMs}ms`);
      } else {
        lastError = err;
      }
      ...
      if (attempt < retries && err.name === 'AbortError') {
        ...
        console.warn(`Claude API timeout, retry ${attempt + 1}/${retries} in ${backoffMs}ms`);
        await sleep(backoffMs);
        continue;
      }
