...
// Initialize SQLite tables (same schema as db.js)
...
export async function query(queryStr, params = []) {
  ...
  if (isPostgres) {
    // Vercel Postgres - use the pool's query method for parameterized queries
    const { sql: sqlTag } = await import('@vercel/postgres');
    // sql.query() accepts standard parameterized queries with $1, $2 etc.
    return await sqlTag.query(queryStr, params);
  } else {
