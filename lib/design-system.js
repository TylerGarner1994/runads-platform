...
export const COMPONENTS = {
  ...
  'bar-chart': ({ title, subtitle, bars, source, max_value }) => {
    const maxVal = max_value || Math.max(...(bars || []).map(b => b.value));
    return `
    <div class="data-viz-container" style="padding: 32px; background: var(--bg-surface); border-radius: var(--radius-lg); border: 1px solid var(--border); margin: 32px 0;">
      ${title ? `<h3 style="font-size: 1.125rem; font-weight: 700; margin-bottom: 4px;">${title}</h3>` : ''}
      ${subtitle ? `<p style="font-size: 0.875rem; color: var(--text-light); margin-bottom: 24px;">${subtitle}</p>` : ''}
      <div style="display: flex; flex-direction: column; gap: 16px;">
        ${(bars || []).map(bar => {
          const pct = Math.round((bar.value / maxVal) * 100);
          const color = bar.highlight ? 'var(--accent)' : 'var(--primary)';
          return `
          <div>
            <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
              <span style="font-size: 0.875rem; font-weight: ${bar.highlight ? '700' : '500'}; color: var(--text);">${bar.label}</span>
              <span style="font-size: 0.875rem; font-weight: 700; color: ${bar.highlight ? 'var(--accent)' : 'var(--text)'};">${bar.display || bar.value}</span>
            </div>
            <div style="height: 12px; background: var(--bg-alt); border-radius: 6px; overflow: hidden;">
              <div style="height: 100%; width: ${pct}%; background: ${color}; border-radius: 6px; transition: width 0.5s ease;"></div>
            </div>
          </div>`;
        }).join('')}
      </div>
      ${source ? `<p style="font-size: 0.75rem; color: var(--text-light); margin-top: 16px; font-style: italic;">Source: ${source}</p>` : ''}
    </div>`;
  },

  'comparison-chart': ({ title, items, highlight_index, source }) => `
    <div class="data-viz-container" style="padding: 32px; background: var(--bg-surface); border-radius: var(--radius-lg); border: 1px solid var(--border); margin: 32px 0;">
      ${title ? `<h3 style="font-size: 1.125rem; font-weight: 700; margin-bottom: 24px; text-align: center;">${title}</h3>` : ''}
      <div style="display: grid; grid-template-columns: repeat(${Math.min((items || []).length, 4)}, 1fr); gap: 16px;">
        ${(items || []).map((item, i) => {
          const isHighlight = i === (highlight_index || 0);
          return `
          <div style="padding: 24px 16px; border-radius: var(--radius-md); text-align: center; border: 2px solid ${isHighlight ? 'var(--accent)' : 'var(--border)'}; background: ${isHighlight ? 'var(--accent-light, rgba(0,0,0,0.02))' : 'var(--bg-surface)'}; position: relative;">
            ${isHighlight ? `<div style="position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background: var(--accent); color: #fff; padding: 2px 12px; border-radius: 10px; font-size: 0.7rem; font-weight: 700; text-transform: uppercase;">Best</div>` : ''}
            <div style="font-size: 2rem; font-weight: 800; color: ${isHighlight ? 'var(--accent)' : 'var(--text)'}; margin-bottom: 8px;">${item.value}</div>
            <div style="font-size: 0.875rem; font-weight: 600; color: var(--text); margin-bottom: 4px;">${item.label}</div>
            ${item.sublabel ? `<div style="font-size: 0.75rem; color: var(--text-light);">${item.sublabel}</div>` : ''}
          </div>`;
        }).join('')}
      </div>
      ${source ? `<p style="font-size: 0.75rem; color: var(--text-light); margin-top: 16px; text-align: center; font-style: italic;">Source: ${source}</p>` : ''}
    </div>`,

  'pull-quote': ({ quote, attribution, stat }) => `
    <blockquote style="margin: 40px 0; padding: 32px 40px; border-left: 4px solid var(--accent); background: var(--bg-alt); border-radius: 0 var(--radius-lg) var(--radius-lg) 0; position: relative;">
      ${stat ? `<div style="font-size: 2.5rem; font-weight: 800; color: var(--accent); margin-bottom: 12px; line-height: 1;">${stat}</div>` : ''}
      <p style="font-size: 1.25rem; font-weight: 500; line-height: 1.5; color: var(--text); font-style: italic; margin: 0;">"${quote}"</p>
      ${attribution ? `<cite style="display: block; margin-top: 12px; font-size: 0.875rem; color: var(--text-light); font-style: normal; font-weight: 600;">— ${attribution}</cite>` : ''}
    </blockquote>`,

  'data-callout': ({ number, label, description, icon }) => `
    <div style="display: flex; align-items: start; gap: 20px; padding: 24px; background: var(--bg-surface); border-radius: var(--radius-lg); border: 1px solid var(--border); margin: 24px 0;">
      <div style="flex-shrink: 0; width: 64px; height: 64px; border-radius: 50%; background: var(--accent-light, rgba(0,0,0,0.05)); display: flex; align-items: center; justify-content: center; font-size: 1.5rem;">${icon || '📊'}</div>
      <div>
        <div style="font-size: 2rem; font-weight: 800; color: var(--accent); line-height: 1;">${number}</div>
        <div style="font-size: 1rem; font-weight: 700; color: var(--text); margin-top: 4px;">${label}</div>
        ${description ? `<p style="font-size: 0.875rem; color: var(--text-light); margin-top: 8px; line-height: 1.5;">${description}</p>` : ''}
      </div>
    </div>`,

  'ingredient-grid': ({ title, ingredients }) => `
    <div style="padding: 32px; background: var(--bg-surface); border-radius: var(--radius-lg); border: 1px solid var(--border); margin: 32px 0;">
      ${title ? `<h3 style="font-size: 1.125rem; font-weight: 700; margin-bottom: 24px;">${title}</h3>` : ''}
      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px;">
        ${(ingredients || []).map(ing => `
        <div style="padding: 20px; background: var(--bg); border-radius: var(--radius-md); border: 1px solid var(--border);">
          <div style="font-size: 1.5rem; margin-bottom: 8px;">${ing.icon || '🌿'}</div>
          <div style="font-weight: 700; font-size: 0.95rem; color: var(--text);">${ing.name}</div>
          ${ing.amount ? `<div style="font-size: 0.8rem; color: var(--accent); font-weight: 600; margin-top: 2px;">${ing.amount}</div>` : ''}
          <p style="font-size: 0.8rem; color: var(--text-light); margin-top: 8px; line-height: 1.4;">${ing.description || ''}</p>
        </div>`).join('')}
      </div>
    </div>`,

  'before-after': ({ before_title, before_items, after_title, after_items }) => `
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin: 32px 0;">
      <div style="padding: 24px; background: #fef2f2; border-radius: var(--radius-lg); border: 1px solid #fecaca;">
        <h4 style="font-size: 1rem; font-weight: 700; color: #991b1b; margin-bottom: 16px; text-transform: uppercase; letter-spacing: 0.05em;">${before_title || 'Before'}</h4>
        ${(before_items || []).map(item => `<div style="padding: 8px 0; border-bottom: 1px solid #fecaca; font-size: 0.9rem; color: #7f1d1d;">✗ ${item}</div>`).join('')}
      </div>
      <div style="padding: 24px; background: #f0fdf4; border-radius: var(--radius-lg); border: 1px solid #bbf7d0;">
        <h4 style="font-size: 1rem; font-weight: 700; color: #166534; margin-bottom: 16px; text-transform: uppercase; letter-spacing: 0.05em;">${after_title || 'After'}</h4>
        ${(after_items || []).map(item => `<div style="padding: 8px 0; border-bottom: 1px solid #bbf7d0; font-size: 0.9rem; color: #14532d;">✓ ${item}</div>`).join('')}
      </div>
    </div>`,

  'drop-cap-section': ({ body, section_label }) => `
    <section class="section">
      <div class="container container-narrow">
        ${section_label ? `<div class="section-label">${section_label}</div>` : ''}
        <div class="article-body" style="margin-top: 20px;">
          <p style="font-size: 1.1rem; line-height: 1.8; color: var(--text-secondary);">
            <span style="float: left; font-size: 4rem; line-height: 0.8; font-weight: 800; color: var(--primary); margin-right: 12px; margin-top: 4px; font-family: Georgia, serif;">${(body || '').charAt(0)}</span>${(body || '').slice(1)}
          </p>
        </div>
      </div>
    </section>`
