...
async function updateJobStep(jobId, stepName, stepData, tokensUsed = 0) {
  ...
  if (isPgAvailable()) {
    const sql = getSql();
    // Dynamic column update based on step name
    const updates = {
      research: () => sql`UPDATE page_generation_jobs SET research_data = ${JSON.stringify(stepData)}, current_step = ${nextStep || 'complete'}, status = ${nextStep ? 'processing' : 'complete'}, tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW() ${nextStep ? sql`` : sql`, completed_at = NOW()`} WHERE id = ${jobId}`,
      brand: () => sql`UPDATE page_generation_jobs SET brand_data = ${JSON.stringify(stepData)}, current_step = ${nextStep || 'complete'}, status = ${nextStep ? 'processing' : 'complete'}, tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW() WHERE id = ${jobId}`,
      strategy: () => sql`UPDATE page_generation_jobs SET strategy_data = ${JSON.stringify(stepData)}, current_step = ${nextStep || 'complete'}, status = ${nextStep ? 'processing' : 'complete'}, tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW() WHERE id = ${jobId}`,
      copy: () => sql`UPDATE page_generation_jobs SET copy_data = ${JSON.stringify(stepData)}, current_step = ${nextStep || 'complete'}, status = ${nextStep ? 'processing' : 'complete'}, tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW() WHERE id = ${jobId}`,
      design: () => sql`UPDATE page_generation_jobs SET design_data = ${JSON.stringify(stepData)}, current_step = ${nextStep || 'complete'}, status = ${nextStep ? 'processing' : 'complete'}, tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW() WHERE id = ${jobId}`,
      factcheck: () => sql`UPDATE page_generation_jobs SET factcheck_data = ${JSON.stringify(stepData)}, current_step = ${nextStep || 'complete'}, status = ${nextStep ? 'processing' : 'complete'}, tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW() WHERE id = ${jobId}`,
      assembly: () => sql`UPDATE page_generation_jobs SET assembly_data = ${JSON.stringify(stepData)}, result_html = ${stepData.html || null}, result_page_id = ${stepData.pageId || null}, current_step = 'complete', status = 'complete', tokens_used = tokens_used + ${tokensUsed}, updated_at = NOW(), completed_at = NOW() WHERE id = ${jobId}`,
    };

    if (updates[stepName]) {
      await updates[stepName]();
    }
  } else {
    // GitHub fallback
    const { data: jobs, sha } = await getGitHubFile('data/jobs.json');
