...
async function ensureStorage() {
  ...
  if (isPgAvailable()) {
    ...
    try {
      ...
      await sql`ALTER TABLE verified_claims ADD COLUMN IF NOT EXISTS confidence_score REAL DEFAULT 0.7`;
      await sql`ALTER TABLE verified_claims ADD COLUMN IF NOT EXISTS claim_type TEXT DEFAULT 'claim'`;
      await sql`ALTER TABLE verified_claims ADD COLUMN IF NOT EXISTS verification_status TEXT DEFAULT 'auto_extracted'`;
      await sql`CREATE INDEX IF NOT EXISTS idx_claims_client ON verified_claims(client_id)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_claims_category ON verified_claims(category)`;
    } catch (e) {
...
// ============================================================
// VERIFIED CLAIMS CRUD
// ============================================================
async function saveVerifiedClaim(claim) {
  await ensureStorage();
  if (isPgAvailable()) {
    await sql`
      INSERT INTO verified_claims (client_id, claim_text, source, category, verified, verified_date,
        confidence_score, claim_type, verification_status, created_at)
      VALUES (${claim.client_id}, ${claim.claim_text}, ${claim.source || null},
        ${claim.category || 'general'}, ${claim.verified !== false},
        ${claim.verified ? new Date().toISOString() : null},
        ${claim.confidence_score || 0.7}, ${claim.claim_type || 'claim'},
        ${claim.verification_status || 'auto_extracted'},
        ${claim.created_at || new Date().toISOString()})`;
    return true;
  }
  // GitHub fallback: store in client claims array
  const client = await getClient(claim.client_id);
  if (client) {
    const claims = client.claims || [];
    claims.push({ ...claim, id: generateId(), created_at: new Date().toISOString() });
    await updateClient(claim.client_id, { claims });
  }
  return true;
}

async function saveVerifiedClaims(clientId, claims) {
  await ensureStorage();
  ...
  for (const claim of claims) {
    try {
      await saveVerifiedClaim({
        client_id: clientId,
        claim_text: claim.claim_text || claim.text || claim.quote,
        source: claim.source || 'website',
        category: claim.category || claim.claim_type || 'general',
        verified: true,
        confidence_score: claim.confidence_score || claim.confidence || 0.7
      });
      saved++;
    } catch (e) {
      console.warn('Failed to save verified claim:', e.message);
    }
  }
  return saved;
}

async function getVerifiedClaims(clientId, options = {}) {
  await ensureStorage();
  if (isPgAvailable()) {
    ...
    if (category) {
      const result = await sql`
        ...
        WHERE client_id = ${clientId} AND category = ${category} AND verified = TRUE
    ...
    const result = await sql`
      SELECT * FROM verified_claims
      WHERE client_id = ${clientId} AND verified = TRUE
      ORDER BY created_at DESC
      LIMIT ${limit || 50}`;
    return result.rows;
  }
  ...
  if (limit) claims = claims.slice(0, limit);
  return claims;
}
...
export {
  ...
  getStats,
  // Verified Claims
  saveVerifiedClaim, saveVerifiedClaims, getVerifiedClaims,
